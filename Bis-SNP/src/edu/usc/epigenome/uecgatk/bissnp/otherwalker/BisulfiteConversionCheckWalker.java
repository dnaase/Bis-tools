...
//	@Argument(fullName = "filter_overlapped_fragment", shortName = "filterOverlappedFragment", doc = "filter out filter Overlapped Fragment when two ends overlapped. Default: false", required = false)
  //  public boolean filterOverlappedFragment = false;
	...
	private BAMFileWriter bamWriter = null;
	private BAMFileWriter bamDirtyWriter = null;
	
	public void initialize(){
		//Pair<Long,Long> initialNum = new Pair<Long,Long>(0L,0L);
		//patternMethyCycleMap = new HashMap<Integer, Pair<Long,Long>>();
		if(cleanBam != null){
			bamWriter = new BAMFileWriter(new File(cleanBam));
			bamWriter.setSortOrder(SAMFileHeader.SortOrder.coordinate, true);
			bamWriter.setHeader(getToolkit().getSAMFileHeader());
		}
		if(dirtyBam != null){
			bamDirtyWriter = new BAMFileWriter(new File(dirtyBam));
			bamDirtyWriter.setSortOrder(SAMFileHeader.SortOrder.coordinate, true);
			bamDirtyWriter.setHeader(getToolkit().getSAMFileHeader());
		}
