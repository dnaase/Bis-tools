...
//	public String reads = null;
	...
	//	}
		...
		if (getToolkit().getArguments().numberOfThreads > 1) {
			...
			if (BAC.OutputMode == OUTPUT_MODE.DEFAULT_FOR_TCGA || BAC.OutputMode == OUTPUT_MODE.EMIT_VARIANT_AND_CYTOSINES ||  BAC.OutputMode == OUTPUT_MODE.NOMESEQ_MODE) {
		...
		if (cpgreads != null) {
				readsWriter.close();
				if (BAC.sequencingMode == MethylSNPModel.GM && gchreads != null) {
	...
	public ContextCondition reduce(BisulfiteVariantCallContext value, ContextCondition sum) {
		...
		for(Entry<String, BisulfiteContextsGenotypeLikelihoods> valueEntrySet : value.getBisulfiteContextsGenotypeLikelihoods().entrySet()){																					// by
			...
			THashMap<String, CytosineParameters> cytosineParameters = bcglTmp.getCytosineParameters();
			THashMap<String, Pair<Integer, Double>> cytosineMethySummaryInEachReadGroup = sum.cytosineMethySummaryByReadsGroup.get(sampleKey);
		...
		for (Entry<String, CytosineParameters> cytosineEntrySet : BAC.cytosineDefined.getContextDefined().entrySet()) {
			...
			for (THashMap<String, Pair<Integer, Double>> cytosineMethyEachRG : sum.cytosineMethySummaryByReadsGroup.values()) {
	...
	public ContextCondition treeReduce(ContextCondition lhs, ContextCondition rhs) {
		...
		if (!rhs.cytosineMethySummaryByReadsGroup.isEmpty()) {
			...
			for (Entry<String, THashMap<String, Pair<Integer, Double>>> cytosineMethySummaryByReadsGroupEntrySet: rhs.cytosineMethySummaryByReadsGroup.entrySet()) {
				...
				//if (lhs.cytosineMethySummaryByReadsGroup.containsKey(sampleKey)) {
					THashMap<String, Pair<Integer, Double>> cytosineMethySummaryLhs = lhs.cytosineMethySummaryByReadsGroup.get(sampleKey);
	...
	private Set<VCFHeaderLine> getHeaderInfo() {
		...
		if (BAC.STANDARD_CONFIDENCE_FOR_EMITTING < BAC.STANDARD_CONFIDENCE_FOR_CALLING)
			headerInfo.add(new VCFFilterHeaderLine(UnifiedGenotyperEngine.LOW_QUAL_FILTER_NAME,
	...
	public void readsDetailReport(BisulfiteVariantCallContext value, boolean posStrand, FormatWriterBase privateWriter) {
		...
		if (rawContext.hasReads()) {
			...
			String refStrand = ".";
			if(BaseUtils.basesAreEqual(value.ref.getBase(), BaseUtils.C)){
				refStrand = "+";
			}else if(BaseUtils.basesAreEqual(value.ref.getBase(), BaseUtils.G)){
				refStrand = "-";
			}else{
				if(value.getSummaryAcrossRG().cytosinePatternStrand == '+'){
					refStrand = refStrand + "+";
	...
	private TreeMap<String, Pair<Integer, Double>> sortHashMapByKeyValue(THashMap<String, Pair<Integer, Double>> input) {
	...
	public static class ContextCondition {
		...
		THashMap<String, Pair<Integer, Double>> cytosineMethySummary = new THashMap<String, Pair<Integer, Double>>();

		// record other kind of cytosine statistics status user provided
		THashMap<String, THashMap<String, Pair<Integer, Double>>> cytosineMethySummaryByReadsGroup = new THashMap<String, THashMap<String, Pair<Integer, Double>>>();
		...
		void makeCytosineMap() {
			...
			for (String sample : samples) {
				THashMap<String, Pair<Integer, Double>> cytosineMethySummaryTmp = new THashMap<String, Pair<Integer, Double>>();
				//for (String cytosineKey : BAC.cytosineDefined.getContextDefined().keySet()) {
				for (Entry<String, CytosineParameters> valueEntrySet : BAC.cytosineDefined.getContextDefined().entrySet()) {
					Pair<Integer, Double> methySummary = new Pair<Integer, Double>(0, 0.0);
