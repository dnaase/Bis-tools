/**
 * basically, it is just a copy from GATK. but have a few adaptations to bisulfite-seq. add 6th
 * state, which is X in bisulfite space when reference is C, base is T..
 */
 ...
 * @time May 4, 2012 5:40:19 PM
 * 
 */
public class BisBAQ extends BAQ {
	...
	private final static double EM = 0.25; // changes 1
	 ...
	 * 
	 * @param d
	 *            gap open prob (not phred scaled!).
	 * @param e
	 *            gap extension prob.
	 * @param b
	 *            band width
	 ...
	 */
	public BisBAQ(final double d, final double e, final int b, final byte minBaseQual, boolean includeClippedBases) {
		cd = d;
		ce = e;
		cb = b;
	...
	/**
	 * Returns a new qual array for read that includes the BAQ adjustment. Does not support
	 * on-the-fly BAQ calculation
	 * 
	 * @param read
	 *            the SAMRecord to operate on
	 * @param overwriteOriginalQuals
	 *            If true, we replace the original qualities scores in the read with their BAQ'd
	 *            version
	 * @param useRawQualsIfNoBAQTag
	 *            If useRawQualsIfNoBAQTag is true, then if there's no BAQ annotation we just use
	 *            the raw quality scores. Throws IllegalStateException is false and no BAQ tag is
	 *            present
	 * @return
	 */
	public static byte[] calcBAQFromTag(SAMRecord read, boolean overwriteOriginalQuals, boolean useRawQualsIfNoBAQTag) {
		...
		if (baq != null) {
			// Offset to base alignment quality (BAQ), of the same length as the
			// read sequence.
			// At the i-th read base, BAQi = Qi - (BQi - 64) where Qi is the
			// i-th base quality.
	...
	public static String encodeBQTag(SAMRecord read, byte[] baq) {
		// Offset to base alignment quality (BAQ), of the same length as the
		// read sequence.
		// At the i-th read base, BAQi = Qi - (BQi - 64) where Qi is the i-th
		// base quality.
		// so BQi = Qi - BAQi + 64
	 ...
	 * Uses the BAQ tag if present already and alwaysRecalculate is false, otherwise fires up the
	 * HMM and does the BAQ on the fly using the refReader to obtain the reference bases as needed.
	 * 
	 * @param read
	 * @param refReader
	...
	public byte[] baqRead(SAMRecord read, IndexedFastaSequenceFile refReader, CalculationMode calculationType, QualityMode qmode) {
		if (DEBUG)
			System.out.printf("BAQ %s read %s%n", calculationType, read.getReadName());

		byte[] BAQQuals = read.getBaseQualities(); // in general we are
													// overwriting quals, so
													// just get a pointer to
													// them
		if (calculationType == CalculationMode.OFF) { // we don't want to do
														// anything
			; // just fall though
			...
			if (calculationType == CalculationMode.RECALCULATE || !hasBAQTag(read)) {
				if (DEBUG)
					System.out.printf("  Calculating BAQ on the fly%n");
				...
				if (hmmResult != null) {
					...
					case DONT_MODIFY:
						...
						break;
					default:
						throw new ReviewedStingException("BUG: unexpected qmode " + qmode);
					}
				}
			} else if (qmode == QualityMode.OVERWRITE_QUALS) { // only makes
																// sense if we
																// are
																// overwriting
																// quals
				if (DEBUG)
					System.out.printf("  Taking BAQ from tag%n");
	...
	public BAQCalculationResult calcBAQFromHMM(byte[] ref, byte[] query, byte[] quals, int queryStart, int queryEnd, boolean negStrand, boolean secondPair) {
		if (queryStart < 0)
			throw new ReviewedStingException("BUG: queryStart < 0: " + queryStart);
		if (queryEnd < 0)
			throw new ReviewedStingException("BUG: queryEnd < 0: " + queryEnd);
		if (queryEnd < queryStart)
			throw new ReviewedStingException("BUG: queryStart < queryEnd : " + queryStart + " end =" + queryEnd);
		...
		hmm_glocal(baqResult.refBases, baqResult.readBases, queryStart, queryLen, baqResult.rawQuals, baqResult.state, baqResult.bq, negStrand, secondPair);
		return baqResult;
	}
	...
	public BAQCalculationResult calcBAQFromHMM(SAMRecord read, byte[] ref, int refOffset) {
		// todo -- need to handle the case where the cigar sum of lengths
		// doesn't cover the whole read
		Pair<Integer, Integer> queryRange = calculateQueryRange(read);
		if (queryRange == null)
			return null; // read has Ns, or is completely clipped away
		...
		BAQCalculationResult baqResult = calcBAQFromHMM(ref, read.getReadBases(), read.getBaseQualities(), queryStart, queryEnd, read.getReadNegativeStrandFlag(), secondPair);
		...
		for (CigarElement elt : read.getCigar().getCigarElements()) {
			...
			case I:
				// todo -- is it really the case that we want to treat I and S
				// the same?
	...
	/*
	 * public static class BAQCalculationResult { public byte[] refBases, rawQuals, readBases, bq;
	 * public int[] state;
	 * 
	 * public BAQCalculationResult(SAMRecord read, byte[] ref) { this(read.getBaseQualities(),
	 * read.getReadBases(), ref); }
	 * 
	 * public BAQCalculationResult(byte[] bases, byte[] quals, byte[] ref) { // prepares data for
	 * calculation rawQuals = quals; readBases = bases;
	 * 
	 * // now actually prepare the data structures, and fire up the hmm bq = new
	 * byte[rawQuals.length]; state = new int[rawQuals.length]; this.refBases = ref; } }
	 */
	@Override
	public BAQCalculationResult calcBAQFromHMM(SAMRecord read, IndexedFastaSequenceFile refReader) {
		// start is alignment start - band width / 2 - size of first I element,
		// if there is one. Stop is similar
		...
		if (stop > refReader.getSequenceDictionary().getSequence(read.getReferenceName()).getSequenceLength()) {
			...
			// now that we have the start and stop, get the reference sequence
			// covering it
	...
	public byte capBaseByBAQ(byte oq, byte bq, int state, int expectedPos) {
		...
		if (isIndel || pos != expectedPos) // we are an indel or we don't align
											// to our best current position
	...
	/**
	 * Returns true if we don't think this read is eligible for the BAQ calculation. Examples
	 * include non-PF reads, duplicates, or unmapped reads. Used by baqRead to determine if a read
	 * should fall through the calculation.
	 * 
	 * @param read
	 * @return
	 */
	...
	public boolean excludeReadFromBAQ(SAMRecord read) {
		// keeping mapped reads, regardless of pairing status, or primary
		// alignment status.
	...
	// ####################################################################################################
	//
	// NOTE -- THIS CODE IS SYNCHRONIZED WITH CODE IN THE SAMTOOLS REPOSITORY.
	// CHANGES TO THIS CODE SHOULD BE
	// NOTE -- PUSHED BACK TO HENG LI
	//
	// ####################################################################################################
	public int hmm_glocal(final byte[] ref, final byte[] query, int qstart, int l_query, final byte[] _iqual, int[] state, byte[] q, boolean negStrand, boolean secondPair) {
		if (ref == null)
			throw new ReviewedStingException("BUG: ref sequence is null");
		if (query == null)
			throw new ReviewedStingException("BUG: query sequence is null");
		if (_iqual == null)
			throw new ReviewedStingException("BUG: query quality vector is null");
		if (query.length != _iqual.length)
			throw new ReviewedStingException("BUG: read sequence length != qual length");
		if (l_query < 1)
			throw new ReviewedStingException("BUG: length of query sequence < 0: " + l_query);
		if (qstart < 0)
			throw new ReviewedStingException("BUG: query sequence start < 0: " + qstart);

		// if ( q != null && q.length != state.length ) throw new
		// ReviewedStingException("BUG: BAQ quality length != read sequence length");
		// if ( state != null && state.length != l_query ) throw new
		// ReviewedStingException("BUG: state length != read sequence length");
		...
		// allocate the forward and backward matrices f[][] and b[][] and the
		// scaling array s[]
		...
		{ // f[1]
			...
			for (k = beg, sum = 0.; k <= end; ++k) {
				int u;
				double e = calcEpsilon(ref[k - 1], query[qstart], _iqual[qstart], negStrand, secondPair);
				u = set_u(bw, 1, k);
				fi[u + 0] = e * bM;
				fi[u + 1] = EI * bI;
				sum += fi[u] + fi[u + 1];
		...
		for (i = 2; i <= l_query; ++i) {
			...
			for (k = beg, sum = 0.; k <= end; ++k) {
				int u, v11, v01, v10;
				double e = calcEpsilon(ref[k - 1], qyi, _iqual[qstart + i - 1], negStrand, secondPair);
				u = set_u(bw, i, k);
		...
		// gdbebug+
		/*
		 * double cac=0.; // undo scaling of forward probabilities to obtain plain probability of
		 * observation given model double[] su = new double[f[l_query].length]; { double sum = 0.;
		 * double[] logs = new double[s.length]; for (k=0; k < logs.length; k++) { logs[k] =
		 * Math.log10(s[k]); sum += logs[k]; } for (k=0; k < f[l_query].length; k++) su[k]=
		 * Math.log10(f[l_query][k])+ sum;
		 * 
		 * cac = MathUtils.softMax(su); } System.out.format("s:%f\n",cac); // gdebug-
		 */
		/*** backward ***/
		// b[l_query] (b[l_query+1][0]=1 and thus \tilde{b}[][]=1/s[l_query+1];
		// this is where s[l_query+1] comes from)
		for (k = 1; k <= l_ref; ++k) {
			int u = set_u(bw, l_query, k);
			double[] bi = b[l_query];
			if (u < 3 || u >= bw2 * 3 + 3)
		...
		for (i = l_query - 1; i >= 1; --i) {
			...
			for (k = end; k >= beg; --k) {
				...
				v10 = set_u(bw, i + 1, k);
				v01 = set_u(bw, i, k + 1);
				double e = (k >= l_ref ? 0 : calcEpsilon(ref[k], qyi1, _iqual[qstart + i], negStrand, secondPair)) * bi1[v11];
				bi[u + 0] = e * m[0] + EI * m[1] * bi1[v10 + 1] + m[2] * bi[v01 + 2]; // bi1[v11]
																						// has been
																						// foled
																						// into e.
		...
		{ // b[0]
			...
			pb = b[0][set_u(bw, 0, 0)] = sum / s[0]; // if everything works as
														// is expected, pb ==
														// 1.0
		}
	...
	protected double calcEpsilon(byte ref, byte read, byte qualB, boolean negStrand, boolean secondPair) {
		if (secondPair) {
			if (negStrand) {
				if (BaseUtils.basesAreEqual(ref, BaseUtils.C) && BaseUtils.basesAreEqual(read, BaseUtils.T)) {
					read = 'x';
				}
			} else {
				if (BaseUtils.basesAreEqual(ref, BaseUtils.G) && BaseUtils.basesAreEqual(read, BaseUtils.A)) {
					read = 'X';
				}
			}
		} else {
	...
	private final Pair<Integer, Integer> calculateQueryRange(SAMRecord read) {
		...
		// iterate over the cigar elements to determine the start and stop of
		// the read bases for the BAQ calculation
		for (CigarElement elt : read.getCigar().getCigarElements()) {
			switch (elt.getOperator()) {
			case N:
			...
			case M:
				...
				// in the else case we aren't including soft clipped bases, so
				// we don't update
		...
		if (queryStop == queryStart) {
			// this read is completely clipped away, and yet is present in the
			// file for some reason
			// usually they are flagged as non-PF, but it's possible to push
			// them through the BAM
			// System.err.printf("WARNING -- read is completely clipped away: "
			// + read.format());
			return null;
		}
	 ...
	 * 
	 * Quick conversion function to maintain internal structure of BAQ calculation on probability
	 * scale, but take the user entered parameter in phred-scale.
	 * 
	...
	private void initializeCachedData() {
		...
		for (char b1 : "ACGTXacgtx".toCharArray()) {
			for (char b2 : "ACGTXacgtx".toCharArray()) { // changes 2
				for (int q = 0; q <= SAMUtils.MAX_PHRED_SCORE; q++) {
