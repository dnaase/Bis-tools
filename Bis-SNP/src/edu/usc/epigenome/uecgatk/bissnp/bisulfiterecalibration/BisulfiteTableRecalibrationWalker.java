...
import edu.usc.epigenome.uecgatk.bissnp.bisulfiteindels.BisBAQMode;
...
public class BisulfiteTableRecalibrationWalker extends ReadWalker<SAMRecord, SAMFileWriter> {
	...
	public static final Pattern COVARIATE_PATTERN = Pattern.compile("^ReadGroup,.*");
	...
	// ///////////////////////////
	private RecalDataManager dataManager; // Holds the data HashMap, mostly used
											// by TableRecalibrationWalker to
											// create collapsed data hashmaps

	/**
	 * By default TableRecalibration emits the OQ field -- so you can go back and look at the
	 * original quality scores, rerun the system using the OQ flags, etc, on the output BAM files;
	 * to turn off emission of the OQ field use this flag.
	 */
	...
	/**
	 * A new bam file in which the quality scores in each read have been recalibrated. The alignment
	 * of the reads is left untouched.
	 */
	@Output(doc = "The output recalibrated BAM file", required = true)
	private StingSAMFileWriter OUTPUT_BAM = null;
	/**
	 * TableRacalibration accepts a --preserve_qscores_less_than / -pQ <Q>flag that instructs
	 * TableRecalibration to not modify quality scores less than <Q>but rather just write them out
	 * unmodified in the recalibrated BAM file. This is useful because Solexa writes Q2 and Q3 bases
	 * when the machine has really gone wrong. This would be fine in and of itself, but when you
	 * select a subset of these reads based on their ability to align to the reference and their
	 * dinucleotide effect, your Q2 and Q3 bins can be elevated to Q8 or Q10, leading to issues
	 * downstream. With the default value of 5, all Q0-Q4 bases are unmodified during recalibration,
	 * so they don't get inappropriately evaluated.
	 */
	...
	/**
	 * By default TableRecalibration applies a Yates' correction to account for overfitting when it
	 * calculates the empirical quality score, in particular, ( # mismatches + 1 ) / ( #
	 * observations + 1 ). TableRecalibration accepts a --smoothing / -sm <int> argument which sets
	 * how many unobserved counts to add to every bin. Use --smoothing 0 to turn off all smoothing
	 * or, for example, --smoothing 15 for a large amount of smoothing.
	 */
	...
	/**
	 * Read in the recalibration table input file. Parse the list of covariate classes used during
	 * CovariateCounterWalker. Parse the CSV data and populate the hashmap.
	 */
	@Override
	public void initialize() {
		...
		// Read in the data from the csv file and populate the data map and
		// covariates list
		...
		try {
				...
				// Read in the covariates that were used from the input file
					...
					if (foundAllCovariates) {
						...
						for (int iii = 0; iii < vals.length - 3; iii++) {
							...
							//	System.err.println(covClass.getSimpleName() + "\t" + vals[iii]);
							...
							if (!foundClass) {
								throw new UserException.MalformedFile(RECAL_FILE, "Malformed input recalibration file. The requested covariate type (" + (vals[iii] + "Covariate")
										+ ") isn't a valid covariate option.");
							}
						}
					}
					...
					if (!foundAllCovariates) {
						...
						if (requestedCovariates.size() < 2) {
							...
							//	System.err.println(cov.toString());
							//System.err.println(requestedCovariates.size());
					...
					addCSVData(RECAL_FILE, line); // Parse the line and add the
													// data to the HashMap
				}
			}
			...
			throw new UserException.MalformedFile(RECAL_FILE, "Error parsing recalibration data at line " + lineNumber
		...
		// Create the tables of empirical quality scores that will be used in
		...
		// Take the header of the input SAM file and tweak it by adding in a new
		// programRecord with the version number and list of covariates that
		// were used
	...
	/**
	 * For each base in the read calculate a new recalibrated quality score and replace the quality
	 * scores in the read
	 * 
	 * @param refBases
	 *            References bases over the length of the read
	 * @param read
	 *            The read to be recalibrated
	...
	public SAMRecord map(ReferenceContext refBases, GATKSAMRecord read, ReadMetaDataTracker metaDataTracker) {
		...
		if (read.getReadLength() == 0) { // Some reads have '*' as the SEQ field
											// and samtools returns length zero.
											// We don't touch these reads.
			return read;
		}
			...
			if (!(RAC.SOLID_NOCALL_STRATEGY == BisulfiteRecalDataManager.SOLID_NOCALL_STRATEGY.THROW_EXCEPTION)) {
				...
				if (badColor) {
					...
					if (RAC.SOLID_NOCALL_STRATEGY == BisulfiteRecalDataManager.SOLID_NOCALL_STRATEGY.LEAVE_READ_UNRECALIBRATED) {
						return read; // can't recalibrate a SOLiD read with no
										// calls in the color space, and the
										// user wants to skip over them
		...
		preserveQScores(originalQuals, recalQuals); // Overwrite the work done
													// if original quality score
													// is too low

		read.setBaseQualities(recalQuals); // Overwrite old qualities with new
											// recalibrated qualities
		if (!DO_NOT_WRITE_OQ && read.getAttribute(RecalDataManager.ORIGINAL_QUAL_ATTRIBUTE_TAG) == null) {
	 ...
	 * 
	 * @param output
	 *            The SAMFileWriter that outputs the bam file
	 */
	@Override
	public void onTraversalDone(SAMFileWriter output) {
		if (numReadsWithMalformedColorSpace != 0) {
			if (RAC.SOLID_NOCALL_STRATEGY == RecalDataManager.SOLID_NOCALL_STRATEGY.LEAVE_READ_UNRECALIBRATED) {
				Utils.warnUser("Discovered " + numReadsWithMalformedColorSpace
						+ " SOLiD reads with no calls in the color space. Unfortunately these reads cannot be recalibrated with this recalibration algorithm "
	 ...
	 *            The read to output
	 * @param output
	 *            The FileWriter to write the read to
	...
	/**
	 * For each covariate read in a value and parse it. Associate those values with the data itself
	 * (num observation and num mismatches)
	 * 
	 * @param line
	 *            A line of CSV data read from the recalibration table data file
	 */
	private void addCSVData(final File file, final String line) {
		...
		if (vals.length != requestedCovariates.size() + 3) { // +3 because of
																// nObservations,
																// nMismatch,
																// and
																// Qempirical
	...
	/**
	 * Implements a serial recalibration of the reads using the combinational table. First, we
	 * perform a positional recalibration, and then a subsequent dinuc correction.
	 * 
	 * Given the full recalibration table, we perform the following preprocessing steps:
	 * 
	 * - calculate the global quality score shift across all data [DeltaQ] - calculate for each of
	 * cycle and dinuc the shift of the quality scores relative to the global shift -- i.e.,
	 * DeltaQ(dinuc) = Sum(pos) Sum(Qual) Qempirical(pos, qual, dinuc) - Qreported(pos, qual, dinuc)
	 * / Npos * Nqual - The final shift equation is:
	 * 
	 * Qrecal = Qreported + DeltaQ + DeltaQ(pos) + DeltaQ(dinuc) + DeltaQ( ... any other covariate
	 * ... )
	 * 
	 * @param key
	 *            The list of Comparables that were calculated from the covariates
	...
	/**
	 * Loop over the list of qualities and overwrite the newly recalibrated score to be the original
	 * score if it was less than some threshold
	 * 
	 * @param originalQuals
	 *            The list of original base quality scores
	 * @param recalQuals
	 *            A list of the new recalibrated quality scores
	 */
