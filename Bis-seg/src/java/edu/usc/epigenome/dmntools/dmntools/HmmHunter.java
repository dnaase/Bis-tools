...
public abstract class HmmHunter {
	...
	@Option(name="-adjWindow",usage="in the binomial mode, define the adjacent window size to compare with (100 means +/-100 bp around segment), default: 100000")
	public int window = 100000;
	...
	public ArrayList<ArrayList<ObservationMethy>> methyValue;
	...
	protected void initiate(String prefix) throws IOException{
		...
		methyValue = new ArrayList<ArrayList<ObservationMethy>>();
	...
	protected void parseBedFile(String bedFile) throws Exception,FileNotFoundException, IOException{
		...
		ArrayList<ObservationMethy> methyValueIsland = new ArrayList<ObservationMethy>();
		...
		while( (line = br.readLine()) != null){
			...
			if(lineNum % PURGE_INTERVAL == 0){
				System.err.println("Begin to process line: " + lineNum);
			}
			...
			ObservationMethy methyTmp = new ObservationMethy(tmp);
			...
			if(!positionIsland.isEmpty() && positionIsland.get((positionIsland.size()-1)).onSameContig(loc) && dist <= gap){
				...
				//if(lineNum % PURGE_INTERVAL == 0){
				//	System.err.println("positionIsland size: " + positionIsland.size());
				//}
				...
				if(positionIsland.size() >= MINIMUM_DATA_POINTS){
					...
					methyValue.add( new ArrayList<ObservationMethy>(methyValueIsland));
					position.add( new ArrayList<GenomeLocus>(positionIsland));
				}
				//System.err.println("position size: " + position.size());
			...
			lineNum++;
			//if(lineNum % PURGE_INTERVAL == 0){
			//	System.err.println("Finishing line: " + lineNum);
			//}
	...
	protected Hmm<ObservationMethy> buildInitHmmRandomlyByBeta(ArrayList<ArrayList<ObservationMethy>> seqs)
	{	
		Hmm<ObservationMethy> hmm = 
				new Hmm<ObservationMethy>(2,new OpdfBetaFactory());
	...
	protected Hmm<ObservationMethy> buildInitHmmByBeta(ArrayList<ArrayList<ObservationMethy>> seqs)
	{	

		KMeansLearner<ObservationMethy> kl = new KMeansLearner<ObservationMethy>(states, new OpdfBetaFactory(),
				seqs);
	...
	protected void segmentHmmState(GenomeLocus[] loci, ObservationMethy[] methyState, int[] hiddenState, NdrForwardBackwardScaledCalculator nfbsc, Hmm<? extends Observation> hmm) throws Exception{
		...
		if(sigTest == 1){
			...
			segmentHmmStateByBetaBinomialTestWithFBSC(loci, methyState, hiddenState, nfbsc,hmm);
		}else{
	...
	abstract protected void segmentHmmStateByBetaBinomialTestWithFBSC(GenomeLocus[] loci, ObservationMethy[] methyState, int[] hiddenState, NdrForwardBackwardScaledCalculator nfbsc, Hmm<? extends Observation> hmm);
	...
	abstract protected void segmentHmmStateByBetaBinomialTestWithFBSC(GenomeLocus[] loci, ObservationMethy[] methyState, int[] hiddenState, BbForwardBackwardScaledCalculator nfbsc, Hmm<? extends Observation> hmm);
	...
	protected double getBetaBinomialSigTest(int k, int n, double pSucess, boolean reverseP){
